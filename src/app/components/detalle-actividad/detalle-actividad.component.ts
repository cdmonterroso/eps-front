...
export class DetalleActividadComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      this.actividadService.getActividad(this.id_actividad).subscribe((res)=>{ //Tambien es válido solamente this.parametro
        ...
        this.descripcion = this.actividad.idnumber.split(','); //Código de unidad y de subunidad escritos en la descripción (idNumber) del curso de Moodle
        console.log("Código de asignación: ", this.descripcion[0]);
        //this.codigoAsignacion = this.descripcion[0]; //El código de asignación es el primer parametro de la descripción (idNumber)
  ...
  asignarCursoAlEstudiante():void{
    ...
    try {
      ...
      if (this.codigoAsignacion.trim() === this.descripcion![0]) { //Si el código de asignación es válido.
        ...
        this.actividadService.getPuestoTrabajo(this.registroUsuario.slice(1) as number).subscribe({
          next:(res)=>{
            ...
            console.log('Se obtuvo correctamente la descripción del curso: ', this.descripcion)
            ...
            if (!this.codigoUnidadTrabajo) { //No se encontró código del trabajador (venía la etiqueta vacía)
              ...
              return;
            } else if (this.descripcion!.length <= 1) { //El curso no tenía el código de unidad en la descripción (no se puede asignar)
              alert('Error al asignar: el curso no especifica a que unidad corresponde');
              return;
            } else {
              if (this.codigoUnidadTrabajo[0] == this.descripcion![1] && this.codigoUnidadTrabajo[1] == this.descripcion![2]) {
      ...
      alert('Error, no se establecieron correctamento los parámetros en la configuración del curso');
    }
